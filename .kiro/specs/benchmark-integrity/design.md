...
Запуск в контейнере `golang:1.23` через `go run ./test/squozebench` (в модуле 2papi, так что берётся тот же релиз squoze, который реально пинит `go.mod` шлюза — сегодня v0.3.0; это важнее, чем тестировать апстримный репозиторий).
