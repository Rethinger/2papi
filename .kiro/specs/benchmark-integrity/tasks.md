...
- [x] **TSK-018**: Перемер накладных расходов шлюза после v0.4.0 и обновление BASELINE матрицы.
  - Requirement: FR-7, FR-12, AC-12.2
  - Deliverables: `test/results/gateway_matrix_report.json`, `test/matrix_compare.mjs` (BASELINE), `test/matrix_gate_selftest.mjs`, `docs/benchmarks.md`, `README.md`, `test/results/README.md`
  - Acceptance: выполнено. `BASELINE.meta` = 2026-09-06 / squoze `v0.4.0` / коммит `ac098a4`, пин доказан на самом бинарнике, который отвечал (`docker create` + `docker cp /app/gateway`, затем `go version -m gateway` → `dep github.com/Rethinger/squoze v0.4.0`; 14 401 688 байт, `linux/amd64`). Строка 438.56 не переразмечена, а **удалена**: она снята на v0.2.0, отдавала `squoze=false` (полная цена анализа, ничего не сжато) и описывает версию на два релиза назад — свежий прогон даёт 8.14 мс на 96.9 КиБ и 74.56 мс на 633.4 КиБ, оба с `applied = squoze=true`. У каждой цифры в `docs/benchmarks.md` указаны железо, размер payload, режим и команда перезапуска.
  - **сверх acceptance**: сравнение перестало быть абсолютным. `matrix_compare.mjs` пересчитывает каждый режим как расстояние до строки `baseline (off)` того же прогона и **гасит** per-mode дрейф, когда пропускная способность baseline вне полосы 0.7–1.4× от записанной (прогон 2026-09-04 даёт 0.55 и 0.38 и отказан как `contended`); дрейф заявляется только при ≥ 20% **и** ≥ 1.0 мс. Сам гейт проверяется — `node test/matrix_gate_selftest.mjs`, 9 проверок зелёные, без Docker и без провайдера.
  - **отозвано заявление**: «headroom поднимает пропускную способность выше baseline на 633 КиБ (97 → 128 rps)» на v0.4.0 **не воспроизводится** — baseline 101 rps обгоняет все работающие режимы (лучший `headroom auto` 87). На 96.9 КиБ эффект настоящий (squoze 346 против 307 при TTFB p95 70 против 99 мс, `headroom balanced` 398), поэтому заявление сужено до этого размера, а не выброшено молча.

- [x] **TSK-019**: Таблица сравнения: свои замеры против заявлений вендоров.
  - Requirement: FR-12, AC-12.1, AC-12.3
  - Deliverables: `docs/benchmarks.md` (§ «Measured here vs vendor claims»), бенч-раздел `README.md`
  - Acceptance: выполнено. 7 строк, у каждой колонка `kind` ∈ **measured here** / **vendor claim** / **unverifiable**, плюс свои условия и источник с датой чтения. Таблица «Design targets (not measurements)» удалена целиком — вместе с целевой строкой 2papi (1-2 / 3-5 / 7 мс, 14 / 30 МБ) и двумя строками «secondhand»; у 9Router оставлена явная запись *no row* с причиной, а не переразмеченные цифры без источника.
  - **отклонение от формулировки acceptance (1/2)**: у bifrost на их же странице в заголовке стоит «**Less than 15µs** added latency per request on average» при 5 000 RPS и 100% success (t3.medium 2 vCPU / 4 ГБ, буфер 15 000, пул 10 000; t3.xlarge 4 vCPU / 16 ГБ, буфер 20 000, пул 15 000), и они сами отмечают, что прогоны t3.xlarge шли на ~10 КБ ответах против ~1 КБ, без перцентилей. Разбивка 11 µs / 59 µs лежит на под-страницах с клиентской навигацией (`extract mode=links` отдаёт только nav и footer), поэтому она **не** приведена, а названа в тексте как отсутствующая — вместо того чтобы подставить её по памяти.
  - **отклонение от формулировки acceptance (2/2)**: сторонний бенчмарк `rbadillap/ai-gateways-benchmark` отдаёт **HTTP 200**, а не 403. Непроверяемым он помечен по другой причине: это харнесс, а не набор результатов — нужны свои платные ключи провайдеров, публикуется только анонимизированный пример `gateway-a` / `gateway-b`, и авторы прямо пишут, что результат есть свойство точки наблюдения, а не глобальный рейтинг.
  - **дополнительно**: страница не считает ни одного отношения «во сколько раз быстрее» между строками и объясняет почему — три разные метрики на трёх разных харнессах; две цифры, ходившие по черновикам сравнения без источника (разбивка bifrost 11 / 59 µs и 4.5 мс p99 для bifrost v1.6.4 из поста LiteLLM), названы отсутствующими явно. Попутно починены четыре дефекта старой страницы: ссылка на несуществующий `curl-format.txt`, команда `go test -bench=.`, которая не совпадает ни с чем (в репозитории нет ни одной Go-бенч-функции), вендорская цифра ~10 нс на выбор ключа, молча приписанная `policy.Auth` этого репозитория, и неверно описанный механизм задержки `test/fakeupstream` (две SSE-рамки со сном 10 мс после каждой — это ~20 мс стрима и **никакой** искусственной задержки до первого байта).
...
| TSK-018 | Complete | BASELINE 2026-09-06 / squoze v0.4.0, host-state гейт, `matrix_gate_selftest.mjs` 9/9; строка 438.56 удалена |
| TSK-019 | Complete | `docs/benchmarks.md` § «Measured here vs vendor claims» — 7 строк с `kind`, целевые строки удалены |
...
Матрица режимов на squoze v0.4.0 (прогон 2026-09-06, он же `BASELINE` в
`test/matrix_compare.mjs`): squoze стоит 8.14 мс на 96.9 КиБ и 74.56 мс на
633.4 КиБ, оба раза с `applied = squoze=true`. Прежняя строка 438.56 снята на
v0.2.0 при `squoze=false` — она удалена из `docs/benchmarks.md`, а не
переразмечена. Дрейф там теперь заявляется только относительно строки
`baseline (off)` того же прогона и только когда хост сопоставим.
