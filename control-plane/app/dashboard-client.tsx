...
export default function DashboardClient() {
  ...
  function submitRouting(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget);
    mutate(() => api('routing', { method: 'POST', body: JSON.stringify({ strategy: form.get('strategy'), sticky_ttl: form.get('sticky_ttl'), max_attempts: Number(form.get('max_attempts') || 2), resilience: { cooldown: form.get('cooldown'), circuit_failures: Number(form.get('circuit_failures') || 3), circuit_reset: form.get('circuit_reset') } }) }));
  }

  function submitRotation(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget); const account = rotating;
    startTransition(async () => {
      setError('');
      try { await api(`accounts/${account.id}`, { method: 'PATCH', body: JSON.stringify({ credential: { api_key: form.get('api_key') } }) }); setRotating(null); await load(); }
      catch (cause) { setError(cause instanceof Error ? cause.message : 'Rotation failed'); }
    });
  }

  function publish() { mutate(() => api('config-versions/publish', { method: 'POST', body: '{}' })); }
  function rollback(version: number) { mutate(async () => { await api('config-versions/rollback', { method: 'POST', body: JSON.stringify({ version }) }); await api('config-versions/publish', { method: 'POST', body: '{}' }); }); }
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {view === 'accounts' && <ResourcePage eyebrow="Provider capacity" title="Accounts" description="Credentials, endpoints, priorities, and routing weight." action={<><button className="secondary" onClick={() => setModal('provider')}><PlusIcon size={16} />Provider</button><button className="primary" onClick={() => setModal('account')}><PlusIcon size={16} />Account</button></>}>
        <div className="table-card"><table><thead><tr><th>Account</th><th>Provider</th><th>Endpoint</th><th>Routing</th><th>Credential</th><th>Status</th><th /></tr></thead><tbody>{data.accounts.map(account => <tr key={account.id}><td><div className="cell-title"><span className="provider-glyph"><ServerIcon size={16} /></span><div><b>{account.display_name}</b><small>{account.name}</small></div></div></td><td>{providerById.get(account.provider_id)?.name ?? 'Unknown'}</td><td><code className="truncate">{account.base_url}</code></td><td>P{account.priority} · W{account.weight}</td><td><Status good={account.secret_present}>{account.secret_present ? `Encrypted v${account.key_version}` : 'Missing'}</Status></td><td><Status good={account.enabled}>{account.enabled ? 'Enabled' : 'Disabled'}</Status></td><td><div className="row-actions"><button className="ghost" onClick={() => setRotating(account)} disabled={isPending}><ShieldLockIcon size={14} />Rotate</button><button className="ghost" onClick={() => toggleAccount(account)} disabled={isPending}>{account.enabled ? 'Disable' : 'Enable'}</button></div></td></tr>)}</tbody></table>{!data.accounts.length && <Empty title="No accounts connected" body="Connect an upstream account to make a model available." onClick={() => setModal('account')} />}</div>
      </ResourcePage>}
    ...
    {modal === 'routing' && <Modal title="Routing policy" onClose={() => setModal(null)}><form onSubmit={submitRouting}><label>Strategy<select name="strategy" defaultValue={data.routing?.strategy ?? 'balanced'}>{['balanced', 'priority', 'weighted'].map(option => <option key={option} value={option}>{option}</option>)}</select></label><div className="form-row"><Field label="Sticky TTL" name="sticky_ttl" defaultValue={data.routing?.sticky_ttl ?? '1h'} /><Field label="Max attempts" name="max_attempts" type="number" defaultValue={String(data.routing?.max_attempts ?? 2)} /></div><div className="form-row three"><Field label="Cooldown" name="cooldown" defaultValue={data.routing?.resilience?.cooldown ?? '30s'} /><Field label="Circuit failures" name="circuit_failures" type="number" defaultValue={String(data.routing?.resilience?.circuit_failures ?? 3)} /><Field label="Circuit reset" name="circuit_reset" defaultValue={data.routing?.resilience?.circuit_reset ?? '1m'} /></div><FormActions pending={isPending} /></form></Modal>}
    {rotating && <Modal title={`Rotate credential · ${rotating.display_name}`} onClose={() => setRotating(null)}><form onSubmit={submitRotation}><Field label="New API key" name="api_key" type="password" placeholder="sk-…" /><FormActions pending={isPending} /></form></Modal>}
  </div>;
}
