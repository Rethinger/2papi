...
import { consumeJsonWhenDue, setJsonWithTtl } from '../redis';
...
type StoredDeviceFlow = { nonce: string; device_code: string; interval: number; next_poll_at_ms: number; created_at: string };
...
export async function startDeviceFlow(ttlSeconds = 900) {
  ...
  const expiresIn = Math.min(Number(body.expires_in ?? ttlSeconds), ttlSeconds);
  const interval = Math.max(1, Number(body.interval ?? 5));
  await setJsonWithTtl(key(session), { nonce, device_code, interval, next_poll_at_ms: Date.now(), created_at: new Date().toISOString() }, expiresIn);
...
export async function pollDeviceFlow(session: string): Promise<{ state: DeviceFlowState; credential?: NormalizedCodexCredential; interval?: number }> {
  const due = await consumeJsonWhenDue<StoredDeviceFlow>(key(session));
  ...
  if (!res.ok) {
    ...
    if (err === 'authorization_pending') { const interval = stored.interval; await setJsonWithTtl(key(session), { ...stored, interval, next_poll_at_ms: Date.now() + interval * 1000 }, ttl); return { state: 'pending', interval }; }
    if (err === 'slow_down') { const interval = Math.max(stored.interval + 5, Number(body.interval ?? 0) || 0); await setJsonWithTtl(key(session), { ...stored, interval, next_poll_at_ms: Date.now() + interval * 1000 }, ttl); return { state: 'slow_down', interval }; }
  ...
  if (!idToken) return { state: 'failed' };
  const identity = await verifyOpenAIIDToken(idToken, stored.nonce);
  if (typeof body.access_token !== 'string' || !body.access_token) return { state: 'failed' };
