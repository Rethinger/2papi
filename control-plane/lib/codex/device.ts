...
export async function pollDeviceFlow(session: string): Promise<{ state: DeviceFlowState; credential?: NormalizedCodexCredential; interval?: number }> {
  ...
  try {
    ...
    await updateLeasedJson(key(session), { ...stored, next_poll_at_ms: Date.now() + stored.interval * 1000 }, ttlMs);
    return { state: 'failed' };
  }
  if (!res.ok) {
    const err = String(body.error ?? 'authorization_pending');
    if (err === 'authorization_pending') { const interval = stored.interval; await updateLeasedJson(key(session), { ...stored, interval, next_poll_at_ms: Date.now() + interval * 1000 }, ttlMs); return { state: 'pending', interval }; }
    if (err === 'slow_down') { const interval = Number(body.interval ?? stored.interval + 5); await updateLeasedJson(key(session), { ...stored, interval, next_poll_at_ms: Date.now() + interval * 1000 }, ttlMs); return { state: 'slow_down', interval }; }
    if (err === 'expired_token') { await updateLeasedJson(key(session), stored, ttlMs, 'delete'); return { state: 'expired' }; }
    if (err === 'access_denied') { await updateLeasedJson(key(session), stored, ttlMs, 'delete'); return { state: 'denied' }; }
    await updateLeasedJson(key(session), { ...stored, next_poll_at_ms: Date.now() + stored.interval * 1000 }, ttlMs);
    return { state: 'failed' };
  }
  const idToken = typeof body.id_token === 'string' ? body.id_token : undefined;
  if (!idToken) { await updateLeasedJson(key(session), { ...stored, next_poll_at_ms: Date.now() + stored.interval * 1000 }, ttlMs); return { state: 'failed' }; }
  let identity;
  try { identity = await verifyOpenAIIDToken(idToken, stored.nonce); } catch { await updateLeasedJson(key(session), { ...stored, next_poll_at_ms: Date.now() + stored.interval * 1000 }, ttlMs); return { state: 'failed' }; }
  if (typeof body.access_token !== 'string' || !body.access_token) { await updateLeasedJson(key(session), { ...stored, next_poll_at_ms: Date.now() + stored.interval * 1000 }, ttlMs); return { state: 'failed' }; }
  await updateLeasedJson(key(session), stored, ttlMs, 'delete');
