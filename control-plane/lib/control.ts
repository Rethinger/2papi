...
// A similar-cache threshold is the Jaccard overlap a near-duplicate must reach.
// The gateway takes it in (0, 1] and rejects an explicit 0, because as a plain
// number 0 is indistinguishable from "not configured" and would silently switch
// off the mode it looks like it enables. Absent/null therefore has to survive as
// absent/null all the way to the snapshot -- never as a coerced 0.
const similarThreshold = () => z.number().positive().max(1).nullish();

const ModelBaseSchema = z.object({ alias: z.string().min(1), upstream_model: z.string().min(1), enabled: z.boolean().default(true), fallbacks: z.array(z.string().min(1)).default([]), input_per_mtok: z.number().nonnegative().default(0), output_per_mtok: z.number().nonnegative().default(0), cache: z.enum(['', 'off', 'exact', 'similar']).optional(), cache_ttl: z.string().optional(), cache_similar_threshold: similarThreshold() });
...
export const ModelPatchSchema = z.object({
  ...
  cache: z.enum(['', 'off', 'exact', 'similar']).optional(),
