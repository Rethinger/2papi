...
function idToken(privateKey: crypto.KeyObject, extra: Record<string, unknown> = {}) {
  return signToken(privateKey, 'kid-1', { iss: process.env.CODEX_AUTH_ORIGIN, aud: 'app_EMoamEEZ73f0CkXaXp7hrann', exp: Math.floor(Date.now() / 1000) + 3600, nonce: 'nonce-1', sub: 'sub-1', chatgpt_account_id: 'acct', email: 'verified@example.com', plan: 'plus', ...extra });
}
...
test('JWT verification validates signature claims nonce and identity fields', async () => {
  ...
  const structured = signToken(privateKey, 'kid-1', { iss: process.env.CODEX_AUTH_ORIGIN, aud: 'app_EMoamEEZ73f0CkXaXp7hrann', exp: Math.floor(Date.now() / 1000) + 3600, sub: 'sub-2', 'https://api.openai.com/auth': { chatgpt_account_id: 'acct-nested', plan_type: 'pro' } });
  ...
  await assert.rejects(() => verifyOpenAIIDToken(signToken(privateKey, 'kid-1', { iss: process.env.CODEX_AUTH_ORIGIN, aud: 'app_EMoamEEZ73f0CkXaXp7hrann', exp: Math.floor(Date.now() / 1000) - 1, chatgpt_account_id: 'acct' })), /expired/);
...
test('browser OAuth requires verified ID-token identity and rejects body fallback', async () => {
  ...
  tokenBody = { access_token: 'at', id_token: idToken(privateKey, { nonce: secondSession.nonce, chatgpt_account_id: 'verified-acct' }), expires_in: 60, account_id: 'body-only' };
...
test('auth.json normalization is strict and refreshes expired imports once', async () => {
  ...
  await assert.rejects(() => parseCodexAuthFile(JSON.stringify({ access_token: 'at', id_token: 'bad.token.value', expires_at: new Date(Date.now() + 60000).toISOString(), chatgpt_account_id: 'acct' })), /id_token|JSON/);
...
test('Device Code polling preserves pending state and authorizes once', async () => {
  ...
  assert.equal(concurrent.filter(result => result.state === 'expired').length, 1);
