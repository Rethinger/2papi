...
test('snapshot compiler emits Go-compatible v1 shape and detached checksum', async () => {
  const query = async (sql: string) => {
    if (sql.startsWith('SELECT a.*, p.adapter FROM accounts')) return { rows: [{ id: '00000000-0000-0000-0000-000000000001', name: 'primary', adapter: 'openai-compatible', base_url: 'http://fake-upstream:9001', enabled: true, priority: 1, weight: 2, max_concurrency: 100, cost: '0.15', credential_revision: 1, secret_record_id: 's1' }] };
    if (sql.startsWith('SELECT a.id, sr.* FROM accounts')) return { rows: [{ id: '00000000-0000-0000-0000-000000000001', key_version: enc.key_version, data_key_nonce: buf(enc.data_key_nonce), data_key_ciphertext: buf(enc.data_key_ciphertext), data_key_tag: buf(enc.data_key_tag), secret_nonce: buf(enc.secret_nonce), secret_ciphertext: buf(enc.secret_ciphertext), secret_tag: buf(enc.secret_tag) }] };
  ...
  assert.equal(snapshot.accounts[0].credential.api_key, 'upstream-primary');
