...
test('snapshot compiler emits Go-compatible v1 shape and checksum metadata', async () => {
  const query = async (sql: string) => {
    ...
    if (sql.startsWith('SELECT * FROM virtual_keys')) return { rows: [{ name: 'dev', key_hash: 'hash', models: ['gpt-dev'], rpm: 60 }] };
    throw new Error(sql);
  };
  ...
  assert.equal(snapshot.virtual_keys[0].key_hash, 'hash');
  assert.equal('key' in snapshot.virtual_keys[0], false);
  assert.equal(snapshot.metadata.checksum, checksum);
});
